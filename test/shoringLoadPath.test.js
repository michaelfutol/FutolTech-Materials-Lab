...
function defaultRecord(overrides = {}) {
  return evaluateShoringSystem({
    slabWidthM: 5, slabLengthM: 5, slabThicknessMm: 125,
    concreteUnitWeightKNM3: 24, plywoodThicknessMm: 12.7, plywoodDensityKgM3: 600,
    rebarAllowanceKgfM2: 20, constructionLiveLoadKgfM2: 250,
    joistTargetSpacingM: 0.3, bearerTargetSpacingM: 0.8, shoreTargetSpacingM: 0.8,
    joistMaterial: coco, joistPreset: wood2x3,
    bearerMaterial: coco, bearerPreset: wood2x4,
    shoreMaterial: coco, shorePreset: wood2x3,
    shoreHeightM: 3, braceMode: 'auto', targetShoreUtilization: 0.8, maximumBraceLevels: 4,
    ...overrides
  });
}
...
test('area-load components include fresh concrete, plywood and pour allowances', () => {
  const load = calculateShoringAreaLoad({ slabThicknessMm: 125, concreteUnitWeightKNM3: 24, plywoodThicknessMm: 12.7, plywoodDensityKgM3: 600, rebarAllowanceKgfM2: 20, constructionLiveLoadKgfM2: 250, miscellaneousLoadKgfM2: 0 });
...
test('bearer point loads come from actual continuous-joist FEM reactions', () => {
  const record = defaultRecord();
  const joistIndex = Math.floor(record.joists.length / 2);
  const bearerIndex = 1;
  const actual = record.bearers[bearerIndex].pointLoads[joistIndex].forceKN;
  const expected = record.joists[joistIndex].supportReactionsKN[bearerIndex].reactionKN;
...
test('manual brace levels use the longest actual segment rather than a hard-coded height', () => {
  assert.deepEqual(normaliseBraceElevations(3, [1.2]), [1.2]);
  assert.throws(() => normaliseBraceElevations(3, [0, 1.5]), /strictly between/);
});

test('auto-suggest checks both capacity and stress utilization', () => {
  const suggestion = suggestBraceElevations({ material: steel, preset: shs50, heightM: 3, axialLoadKN: 15, eccentricityMm: 10, targetUtilization: 0.8, maximumBraceLevels: 4 });
  const selected = suggestion.recommended;
  selected.elevationsM.forEach((elevation, index) => assert.ok(Math.abs(elevation - 3 * (index + 1) / (selected.count + 1)) < 1e-10));
