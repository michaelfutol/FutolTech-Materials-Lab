...
function makeFixture({ slopeDeg = 25, bayStartM = 0.4, baySpanM = 4, firstBandWidthM = 1, effectiveWidthSelection = 'actual-tributary-width', heightM = 8.82 } = {}) {
  ...
  const zones = resolveWindRoofZoneGeometry({
    ...
    symmetricGableConfirmed: true, symmetricGableSourceReference: 'Roof section confirms symmetric gable', roofPlane: 'slope-a', roofBayStartAlongRidgeM: bayStartM, roofBaySpanM: baySpanM,
    roofBayGeometrySourceReference: 'Accepted Roof Bay geometry', purlinTributaryBands: bands,
    edgeDimensionHeightType: slopeDeg <= 10 ? 'eave-height' : 'mean-roof-height', edgeDimensionHeightM: slopeDeg <= 10 ? 8 : heightM, edgeDimensionHeightSourceReference: 'Accepted roof-height record'
  });
  const effectiveArea = resolveRoofPurlinEffectiveWindArea({
    ...
    purlinGeometrySourceReference: 'Selected P1 physical Roof Bay geometry', effectiveWidthSelection, effectiveWidthSelectionSourceReference: 'Engineer-selected NSCP C&C effective-area rule path'
  });
  const record = resolveWindRoofExternalGcp({ windRoofZoneGeometry: zones, roofPurlinEffectiveWindArea: effectiveArea, targetPurlinBandLabel: 'P1', codeFigureSourceReference: 'Authorized NSCP 2015 Figure 207E.4-2B/2C project check', curveEquationSourceReference: 'ASCE 7-10 Wind Loads Guide Table G2-3/G2-4 equation cross-check' });
  return { context, zones, effectiveArea, record };
}
function caseByType(record, type) { return record.coefficientCases.find((item) => item.type === type); }
...
test('2B benchmark evaluates 4.0 m2 purlin effective area on log10 curve and preserves separate zone coefficients', () => {
  ...
  const field = caseByType(record, 'field'); const edge = caseByType(record, 'edge'); const corner = caseByType(record, 'corner');
...
test('curve evaluation holds the <=10 ft2 and >=100 ft2 graph plateaus', () => {
  ...
  assert.equal(low.coefficientArea.regime, 'LOW_AREA_PLATEAU'); assert.equal(caseByType(low, 'edge').positiveGCp, 0.5); assert.equal(caseByType(low, 'edge').negativeGCp, -1.7);
  const high = makeFixture({ baySpanM: 4, firstBandWidthM: 3, bayStartM: 2 }).record;
  assert.equal(high.coefficientArea.regime, 'HIGH_AREA_PLATEAU'); assert.equal(caseByType(high, 'edge').positiveGCp, 0.3); assert.equal(caseByType(high, 'field').negativeGCp, -0.8);
});
...
test('zone geometry and effective-area records must describe the same pressure context and physical purlin band', () => {
  ...
  const mismatchedContextArea = resolveRoofPurlinEffectiveWindArea({ windPressureContextAcceptance: wrongContext, purlinSpanM: 4, actualTributaryWidthM: 1, purlinGeometrySourceReference: 'Mismatched context geometry', effectiveWidthSelection: 'actual-tributary-width', effectiveWidthSelectionSourceReference: 'Explicit test path' });
  assert.throws(() => resolveWindRoofExternalGcp({ windRoofZoneGeometry: fixture.zones, roofPurlinEffectiveWindArea: mismatchedContextArea, targetPurlinBandLabel: 'P1', codeFigureSourceReference: 'Authorized NSCP figure', curveEquationSourceReference: 'ASCE guide equation' }), /exact same accepted wind pressure context/);
  const wrongWidthArea = resolveRoofPurlinEffectiveWindArea({ windPressureContextAcceptance: fixture.context, purlinSpanM: 4, actualTributaryWidthM: 0.8, purlinGeometrySourceReference: 'Wrong width geometry', effectiveWidthSelection: 'actual-tributary-width', effectiveWidthSelectionSourceReference: 'Explicit test path' });
  assert.throws(() => resolveWindRoofExternalGcp({ windRoofZoneGeometry: fixture.zones, roofPurlinEffectiveWindArea: wrongWidthArea, targetPurlinBandLabel: 'P1', codeFigureSourceReference: 'Authorized NSCP figure', curveEquationSourceReference: 'ASCE guide equation' }), /actual tributary width must match/);
});

test('unsupported purlin label and mean roof height above 18 m fail visibly', () => {
  const fixture = makeFixture();
  assert.throws(() => resolveWindRoofExternalGcp({ windRoofZoneGeometry: fixture.zones, roofPurlinEffectiveWindArea: fixture.effectiveArea, targetPurlinBandLabel: 'P99', codeFigureSourceReference: 'Authorized NSCP figure', curveEquationSourceReference: 'ASCE guide equation' }), /was not found/);
...
test('external GCp record round-trips deterministically and rejects coefficient mutation', () => {
  const { record } = makeFixture();
  const first = serializeWindRoofExternalGcp(record); const second = serializeWindRoofExternalGcp(parseWindRoofExternalGcp(first));
  assert.equal(second, first);
  const mutated = structuredClone(record); mutated.coefficientCases[0].negativeGCp -= 0.1;
...
test('GCp resolution cannot silently promote pressure, sheet/fastener area, routing or purlin capacity', () => {
  for (const flag of ['externalPressureTermImplemented','externalInternalPressureCombinationImplemented','roofSheetEffectiveWindAreaImplemented','fastenerEffectiveWindAreaImplemented','codeDerivedRoofPressureImplemented','roofBayCodePressureRoutingImplemented','purlinCapacityPromotionImplemented']) {
    const { record } = makeFixture(); record.implementation[flag] = true;
