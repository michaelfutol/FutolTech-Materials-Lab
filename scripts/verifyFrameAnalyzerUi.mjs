...
const work = await mkdtemp(join(tmpdir(), 'ft-frame-ui-'));
...
try {
  ...
  for (let i = 0; i < 120; i += 1) {
    const state = await evaluate(cdp, `({ready:document.readyState,state:document.documentElement.dataset.frameState,cards:document.querySelectorAll('#frameResultCards .result-card').length})`);
  ...
  const rigid = await evaluate(cdp, `({banner:document.querySelector('#frameStateBanner').textContent,cards:document.querySelector('#frameResultCards').textContent,trace:document.querySelector('#frameCalculationTrace').textContent,connections:document.querySelectorAll('#frameConnectionBody tr').length,diagram:document.querySelectorAll('#frameDiagram .frame-member-deformed').length,error:document.querySelector('#frameErrorBanner').textContent})`);
  ...
  await evaluate(cdp, `(() => { const jt=document.querySelector('#frameJointTypeSelect'); jt.value='spring'; jt.dispatchEvent(new Event('change',{bubbles:true})); document.querySelector('#frameSpringStiffnessInput').value='10'; document.querySelector('#frameRunButton').click(); })()`);
  await sleep(300);
  ...
  await evaluate(cdp, `(() => { const analysis=document.querySelector('#frameAnalysisSelect'); analysis.value='pdelta'; analysis.dispatchEvent(new Event('change',{bubbles:true})); document.querySelector('#frameGravityLoadInput').value='50'; document.querySelector('#frameRunButton').click(); })()`);
  await sleep(350);
  ...
  await evaluate(cdp, `(() => { const analysis=document.querySelector('#frameAnalysisSelect'); analysis.value='redistribution'; analysis.dispatchEvent(new Event('change',{bubbles:true})); document.querySelector('#frameGravityLoadInput').value='0'; document.querySelector('#frameMomentLimitInput').value='0.001'; document.querySelector('#frameResidualRatioInput').value='0.2'; document.querySelector('#frameRunButton').click(); })()`);
  await sleep(400);
  ...
  console.log('NF-001 rigid, explicit semi-rigid, P-Delta and connection-redistribution QA passed in real Chromium.');
} finally {
