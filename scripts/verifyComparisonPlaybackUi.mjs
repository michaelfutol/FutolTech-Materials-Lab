...
const server=createServer((req,res)=>{const raw=decodeURIComponent((req.url||'/').split('?')[0]),rel=raw==='/'?'/c-purlin-test.html':raw,file=normalize(join(root,rel));if(!file.startsWith(root))return res.writeHead(403).end();res.setHeader('Content-Type',mime.get(extname(file))||'application/octet-stream');const s=createReadStream(file);s.on('error',()=>res.writeHead(404).end());s.pipe(res);});
...
try{
  proc=spawn(chromePath(),['--headless=new','--no-sandbox','--disable-gpu','--disable-dev-shm-usage','--remote-debugging-port=0',`--user-data-dir=${profile}`,`http://127.0.0.1:${port}/c-purlin-test.html?build=sim2-ci`],{stdio:'ignore'});
  ...
  let ready=false;let lastState=null;for(let i=0;i<220;i+=1){const s=await evalv(cdp,`({ready:document.readyState,panel:!!document.querySelector('.compare-shell [data-comparison-playback]'),benchmark:!!document.querySelector('.compare-shell [data-cp-benchmark]'),cpBench:!!document.querySelector('.compare-shell [data-c-purlin-physics-bench]'),experience:document.documentElement.dataset.comparisonExperience,slots:[0,1,2].map(i=>!!document.querySelector('[data-slot-material="'+i+'"]')&&!!document.querySelector('[data-slot-preset="'+i+'"]')&&!!document.querySelector('[data-slot-orientation="'+i+'"]')),third:!!document.querySelector('[data-slot-enable="2"]'),root:!!document.getElementById('compareLoadEquivalent')})`);lastState=s;if(s?.ready==='complete'&&s.panel&&s.benchmark&&s.cpBench&&s.experience==='c-purlin'&&s.slots.every(Boolean)&&s.third&&s.root){ready=true;break;}await sleep(100);}if(!ready)throw new Error(`Dedicated C-purlin playback did not mount: ${JSON.stringify(lastState)}`);

  await evalv(cdp,`document.querySelector('.compare-shell [data-cp-benchmark]').click()`);await sleep(700);
  const configured=await evalv(cdp,`(() => { const panel=document.querySelector('.compare-shell [data-comparison-playback]'); return {third:document.querySelector('[data-slot-enable="2"]')?.checked,presets:[0,1].map(i=>document.querySelector('[data-slot-preset="'+i+'"]')?.value),angles:[0,1].map(i=>document.querySelector('[data-c-purlin-orientation-display="'+i+'"]')?.value),load:document.querySelector('#compareLoadInput').value,unit:document.querySelector('#compareLoadUnitSelect').value,livePanels:document.querySelectorAll('.compare-shell [data-comparison-playback]').length,allPanels:document.querySelectorAll('[data-comparison-playback]').length,cards:panel?.querySelectorAll('[data-playback-member]').length ?? 0}; })()`);
  ...
  await evalv(cdp,`document.querySelector('.compare-shell [data-cp-step]').click()`);await sleep(150);
  const stepped=await evalv(cdp,`Number(document.querySelector('.compare-shell [data-cp-scrub]').value)`);if(!(stepped>500))throw new Error(`STEP did not advance synchronized progress: ${stepped}`);

  await evalv(cdp,`(() => { const s=document.querySelector('.compare-shell [data-cp-scrub]');s.value='1000';s.dispatchEvent(new Event('input',{bubbles:true})); })()`);await sleep(250);
  await evalv(cdp,`document.querySelector('.compare-shell [data-cp-export]').click()`);await sleep(120);
  ...
  console.log('Dedicated C-purlin 0° vs 90° synchronized playback + JSON export QA passed in real Chromium.');
