...
try{
  ...
  let ready=false;for(let i=0;i<180;i+=1){const s=await evalv(cdp,`({ready:document.readyState,panel:!!document.querySelector('[data-comparison-playback]'),benchmark:!!document.querySelector('[data-cp-benchmark]'),cards:document.querySelectorAll('#compareSelectors .compare-selector-card').length})`);if(s?.ready==='complete'&&s.panel&&s.benchmark&&s.cards===3){ready=true;break;}await sleep(100);}if(!ready)throw new Error('Side-by-side playback did not mount.');
