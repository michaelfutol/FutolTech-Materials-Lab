...
if (root) {
  ...
  if (baseCanvas && summary && body && actions) {
    ...
    function drawExploded(model) {
      ...
      const layer=(y,label,color,width=5)=>{ctx.strokeStyle=color;ctx.lineWidth=width;ctx.beginPath();ctx.moveTo(x0,y);ctx.lineTo(x1,y);ctx.stroke();ctx.fillStyle=p.text;ctx.fillText(label,15,y+5);};
