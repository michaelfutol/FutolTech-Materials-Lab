...
13. [~] **Roof Resilience Physics M4** — ACTIVE. PR #136 merged as `ec5e7c99994a4c5d52bdf2ad90a1790b13a8e181`; PR #137 merged as `6e5de1e29373c0657f7bb42fe16a415abca0229b`; PR #138 merged as `471a62cbe305e385a9542f9f3324e251c06a7981`; PR #139 merged as `91400114a54cc074d7763c7f5df4eb0f37165245`; PR #140 merged as `dfe58947f09fbd214f590b999ac02886419677b6`; PR #141 merged as `6d84e1be0db8853ae503603b96edfd099171faca`; PR #142 merged as `a9a48d139e0b9098a77be3cc9a33ab32b16c866e` after exact documentation-updated head `65a1c371…` passed 46/46. PR #143 now bridges #141 evidence to #142 physical sheet pieces with exact project applicability checks while keeping capacity-row selection, basis alignment and utilization unresolved; preliminary exact head `294335cc…` passed 46/46 and the exact documentation-updated head remains the merge gate.
...
## M4 ACTIVE — PR #136/#137/#138/#139/#140/#141/#142 merged / PR #143 project-applicability candidate
...
- [x] PR #141 adds versioned `futoltech.roof-sheet-positive-pressure-capacity-evidence/1` acceptance for source-backed panel-capacity rows applicable to loading pushing the roof sheet toward its supports and merged as `6d84e1be0db8853ae503603b96edfd099171faca`.
...
- [x] Project sheet continuity, end laps, actual panel span count and exact source-row project applicability are deliberately not inferred from purlin locations; product applicability alone never creates a project PASS.
...
- [x] Exact documentation-updated PR #141 head `1e4843d061d7e6f1e6f503998e1c4726f2e32703` passed the complete 46/46 Engineering Checks suite before merge.
- [x] PR #142 adds versioned `futoltech.roof-sheet-panel-span-continuity/1` for explicit project panel runs, physical sheet pieces, real purlin-support spans and end-lap geometry and merged as `a9a48d139e0b9098a77be3cc9a33ab32b16c866e`.
- [x] #142 defines roof-sheet structural spans upslope between successive physical purlin support lines; rafter-to-rafter Roof Bay x-span is not reused as panel structural span.
- [x] Panel runs must partition the full Roof Bay width without x-direction gaps or overlaps.
- [x] Each physical sheet piece deterministically derives its exact support sequence, successive nonuniform span lengths, span count/type and eave/ridge overhang geometry from the accepted purlin stations.
- [x] Continuity is limited to one explicitly identified physical sheet piece across the supports it actually crosses.
- [x] Successive physical sheet pieces require positive end-lap overlap; gaps and zero-overlap butt joints are rejected.
- [x] Every end lap retains exact overlap interval/length, detail source and optional identified purlin support within the lap.
- [x] Separate overlapping sheet pieces are a monolithic-continuity break; overlap/fastening alone does not silently create continuous-sheet action.
- [x] An end lap with no identified purlin support may be recorded but remains visibly unresolved/unrated.
- [x] Deterministic validation rejects run gaps/overlaps, piece gaps/butt joints, wrong lap length, unknown/out-of-lap supports, insufficient support geometry, derived-span mutation and fake project applicability/utilization/PASS promotion.
- [x] Exact documentation-updated PR #142 head `65a1c371af648fb694e08303356801a99158852b` passed the complete 46/46 Engineering Checks suite with no rerun and no branch mutation before merge.
- [~] PR #143 candidate adds versioned `futoltech.roof-sheet-positive-pressure-project-applicability/1` for exact source-to-project applicability screening.
- [x] #143 consumes exact #142 panel geometry and its embedded #141 evidence instead of duplicating either source.
- [x] #141 product applicability must be complete before any source row can become project-applicable.
- [x] Every accepted source row is evaluated independently against every physical sheet piece.
- [x] Exact piece span type must be explicitly covered by source applicability.
- [x] Every actual purlin-to-purlin support spacing must lie inside the source-backed range; unequal spacings are never averaged to create a match.
- [x] Overhang is piece-specific: eave/ridge edge pieces and interior end-lapped pieces retain their own physical conditions.
- [x] `toward-support` and an explicit source-referenced target load category must both be covered.
- [x] A live-load/deflection source row cannot be reused for positive wind unless source applicability explicitly includes `positive-wind`.
- [x] `source-defined` span/overhang labels are literal and are never treated as wildcards.
- [x] Supplied source applicability must cover its own source row; contradictory source metadata is rejected.
- [x] Per-piece result states distinguish `PROJECT_APPLICABILITY_COMPLETE`, incomplete reference-only evidence and explicit `PROJECT_APPLICABILITY_EXCLUDED`.
- [x] Different source rows may cover different physical pieces; #143 reports coverage without selecting a governing row.
- [x] Capacity-row selection, interpolation/extrapolation, demand/capacity basis alignment, positive-pressure panel utilization, end-lap capacity, local contact, purlin local bearing/web crippling, screw compression/bearing/shear and `roofSystemPass` remain unresolved.
- [x] Preliminary exact PR #143 implementation/test/doc head `294335cc1c29e43943cadf876ab21b2ed2ec0a14` passed the complete 46/46 Engineering Checks suite with no rerun or branch mutation.
- [ ] PR #143 merge gate: all four authority records synchronized, then only the exact documentation-updated head may merge after a fresh complete 46/46 Engineering Checks pass.
- [ ] Next M4 slice: select explicitly project-applicable capacity evidence for each physical sheet-piece use case and align demand/capacity engineering bases before any numerical positive-pressure utilization; no convenience interpolation/extrapolation.
...
- 2026-08-26 — PR #141 accepts source-backed roof-sheet positive-pressure panel capacity evidence only after exact product/profile/BMT/Fy/Fu applicability checks. Source span type/support spacing/overhang, capacity basis, deflection limit and covered limit states are preserved, but actual project panel continuity/end laps/span count are not inferred; project applicability and every positive-pressure utilization/PASS remain unresolved. Exact documentation-updated head `1e4843d061d7e6f1e6f503998e1c4726f2e32703` passed 46/46 and it merged as `6d84e1be0db8853ae503603b96edfd099171faca`.
- 2026-08-26 — PR #142 accepts explicit project panel runs, physical sheet-piece extents, real purlin support sequences/spans and end-lap geometry. Structural span is upslope purlin-to-purlin; continuity is limited to one physical sheet piece; separate overlapped pieces remain a monolithic-continuity break. Missing lap-support identification may remain unresolved, and source-row project applicability/utilization/PASS stay blocked. Exact documentation-updated head `65a1c371af648fb694e08303356801a99158852b` passed 46/46 with no rerun/branch mutation and it merged as `a9a48d139e0b9098a77be3cc9a33ab32b16c866e`.
- 2026-08-26 — PR #143 compares exact #142 physical sheet-piece span type, every actual support spacing, piece-specific overhang, `toward-support` direction and an explicit target load category against #141 source applicability. It forbids average-spacing matches, `source-defined` wildcards and live-load→positive-wind reuse without explicit source coverage; row selection, interpolation/extrapolation, basis alignment and utilization remain unresolved. Preliminary exact head `294335cc1c29e43943cadf876ab21b2ed2ec0a14` passed 46/46 with no rerun or branch mutation; the exact documentation-updated head remains the merge gate.