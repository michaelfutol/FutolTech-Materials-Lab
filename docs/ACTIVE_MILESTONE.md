...
Current M3 task: **internal-pressure velocity-pressure selection + signed `qi(GCpi)` term — PR #121 candidate**.

Implemented in the PR #121 candidate:
...
Next step after PR #121 is final-head green and merged: begin the source-backed external roof pressure coefficient slice, still without routing code pressure into Roof Bay until the complete coefficient/zoning/combination chain is verified.

Permanent M3 rule: even a resolved internal-pressure term is still not final roof pressure. Manual pressure remains the active Roof Bay path until the complete internal/external coefficient, zoning, and pressure-combination chain is verified.
